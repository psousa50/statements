...
class TestExistingTransactionsCategorizer:
    ...
    ExpectedCategorization = namedtuple(
        "ExpectedCategorization", ["category_id", "confidence"]
    )
    ...
    def fallback_categorizer(self):
        ...
        categorizer.categorize_transaction.side_effect = (
            categorize_transaction_side_effect
        )
        return categorizer

    @pytest.mark.asyncio
    async def test_categorize_transaction_with_existing_match(
        self, transactions_repository, fallback_categorizer
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_transaction_without_existing_match(
        self, transactions_repository, fallback_categorizer
    ):
        categorizer = ExistingTransactionsCategorizer(
            transactions_repository=transactions_repository,
    ...
    async def test_categorize_multiple_transactions(
        ...
        assert (
            transactions_repository.get_unique_normalized_descriptions.call_count == 1
        )

        fallback_categorizer.categorize_transaction.assert_called_once()
        assert len(fallback_categorizer.categorize_transaction.call_args[0][0]) == 1
        assert (
            fallback_categorizer.categorize_transaction.call_args[0][0][
                0
