...
def upgrade() -> None:
    ...
    result = connection.execute(
        text("SELECT EXISTS (SELECT FROM sources WHERE name = 'unknown')")
    )
    ...
    if not default_source_exists:
        op.execute(
            text(
                "INSERT INTO sources (name, description) VALUES ('unknown', 'Default source for transactions with unknown origin')"
            )
        )
    ...
    result = connection.execute(
        text("SELECT DISTINCT source_id FROM transactions WHERE source_id != 'unknown'")
    )
    ...
    for name in source_names:
        result = connection.execute(
            text(f"SELECT EXISTS (SELECT FROM sources WHERE name = '{name}')")
        )
        ...
        result = connection.execute(
            text(f"SELECT id FROM sources WHERE name = '{name}'")
        )
        ...
        op.execute(
            text(
                f"UPDATE transactions SET source_id = '{source_id}' WHERE source_id = '{name}'"
            )
        )
    ...
    op.execute(
        text(
            f"UPDATE transactions SET source_id = '{default_source_id}' WHERE source_id = 'unknown'"
        )
    )

    # Create a temporary column to store the integer source_id
    op.add_column(
        "transactions", sa.Column("temp_source_id", sa.Integer(), nullable=True)
    )
    ...
    op.alter_column(
        "transactions", "temp_source_id", new_column_name="source_id", nullable=False
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "transactions", sa.Column("temp_source_id", sa.String(), nullable=True)
    )
    ...
    for source_id, source_name in sources.items():
        op.execute(
            text(
                f"UPDATE transactions SET temp_source_id = '{source_name}' WHERE source_id = {source_id}"
            )
        )
