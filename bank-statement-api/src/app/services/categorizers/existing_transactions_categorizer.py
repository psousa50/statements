...
class ExistingTransactionsCategorizer(TransactionCategorizer):
    ...
    async def categorize_transaction(
        ...
        description_category_pairs = self.transactions_repository.get_unique_normalized_descriptions(100)
        for desc, cat_id in description_category_pairs:
            print(f"Found existing category for {desc}")
        
        description_to_category = {desc: cat_id for desc, cat_id in description_category_pairs}
        
        for transaction in transactions:
            if transaction.normalized_description in description_to_category:
                print(f"Found existing category for {transaction.normalized_description}")
                category_id = description_to_category[transaction.normalized_description]                
                results.append(
                    CategorizationResult(
        ...
        if transactions_for_fallback:
            print(f"Falling back to {self.fallback_categorizer.__class__.__name__} for {len(transactions_for_fallback)} transactions")
