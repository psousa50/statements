...
class CategoryRouter:
    def __init__(
        ...
        on_change_callback: Optional[CategoryChangeCallback] = None,
    ):
    ...
    async def import_categories_from_csv(self, file: UploadFile = File(...)):
        ...
        try:
            ...
            for row in csv_reader:
                ...
                main_category = self.categories_repository.get_by_name(
                    main_category_name
                )

                if not main_category:
                    main_category = Category(
                        category_name=main_category_name, parent_category_id=None
                    )
                    main_category = self.categories_repository.create(
                        main_category, autocommit=True
                    )
            ...
            return {
                "message": f"Successfully imported {len(categories_created)} categories"
            }

        except Exception as e:
            self.categories_repository.rollback()
            raise HTTPException(
                status_code=400, detail=f"Error importing categories: {str(e)}"
            )
